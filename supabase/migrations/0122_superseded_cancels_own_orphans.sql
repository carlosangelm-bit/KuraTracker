-- =============================================================================
-- 0122_superseded_cancels_own_orphans.sql — La rama SUPERSEDIDA debe cancelar los
-- derechos ÚNICOS de la suscripción vieja (fuga del §7.10).
-- =============================================================================
-- 0119 agregó la guarda de supersedida: si un evento 'canceled' de una suscripción
-- vieja (A) llega DESPUÉS de que otra (B) quedó registrada como la viva, se devolvía
-- 'superseded' SIN tocar derechos, para no pisar con (A,canceled) las filas que B ya
-- había escrito (comparten (org,kind,key)).
--
-- Pero eso deja una fuga: los derechos ÚNICOS de A —los que B NO incluye, p.ej.
-- module:admin— siguen tagueados con A y quedan ACTIVOS sin pago. El barrido acotado
-- a B nunca los toca (están tagueados A), y la rama de A no tocaba nada. Un centro
-- que recontrata más barato se queda con un módulo del plan viejo, gratis.
--
-- Arreglo: en la rama de supersedida, cancelar los derechos que SIGAN tagueados con A.
-- Los que B compartía ya están tagueados B (el upsert de B los pisó) y NO matchean →
-- sobreviven; los únicos de A sí se cancelan. No se corre el upsert de A (pisaría B).
--
-- Redefine apply_stripe_subscription_event; el resto del cuerpo es idéntico a 0119.
-- Aditivo (0119 ya aplicada): no se edita, se create-or-replace.
-- =============================================================================
...
begin
  ...
  -- El evento ya quedó en stripe_events (arriba) → no se reintenta.
  ...
  if v_status = 'canceled'
    ...
    -- A quedó supersedida por otra suscripción viva (B, ya registrada). Se cancelan
    -- los derechos que SIGAN tagueados con A: los que B compartía ya están tagueados
    -- B (el upsert de B los pisó) y NO matchean → sobreviven; los ÚNICOS de A (p.ej.
    -- module:admin que B no incluye) sí se cancelan. Sin esto quedaban ACTIVOS sin
    -- pago (fuga del §7.10). NO se corre el upsert de A (pisaría los de B). La columna
    -- de organizations ya apunta a B y no se toca.
