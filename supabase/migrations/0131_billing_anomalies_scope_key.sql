-- =============================================================================
-- 0131_billing_anomalies_scope_key.sql — Una fila por clave que deriva.
-- =============================================================================
-- 0130 abrió catalog_price_drift con upsert por (org, kind): una sola fila por centro,
-- así que varios precios derivando a la vez se pisaban (la última clave ganaba el
-- detail). Se agrega un DISCRIMINADOR dentro del kind (scope_key) para que cada precio
-- que deriva tenga su propia fila, su propio seen_count y se resuelva por separado.
--
-- El default '' deja las filas y los kinds existentes EXACTAMENTE como están:
-- two_live_subscriptions no escribe scope_key → queda '' y su dedup por par de subs no
-- cambia. Solo catalog_price_drift lo usa (escribe el lookup_key).
-- =============================================================================

alter table public.billing_anomalies
  add column if not exists scope_key text not null default '';

comment on column public.billing_anomalies.scope_key is
  'Discriminador DENTRO del kind: separa filas del mismo (org, kind) cuando una '
  'anomalía puede repetirse por sub-concepto. catalog_price_drift guarda aquí el '
  'lookup_key (una fila por precio que deriva). Kinds sin sub-concepto lo dejan '''' — '
  'el siguiente kind que lo necesite usa esta columna, no improvisa sobre sub_low.';

-- Recrear la clave de dedup incluyendo scope_key. Con '' por defecto, las filas y el
-- comportamiento de los kinds existentes no cambian.
drop index if exists public.uq_billing_anomalies_key;
...
-- apply_stripe_subscription_event: catalog_price_drift ahora escribe scope_key =
-- lookup_key, y AMBOS upserts apuntan al índice único de 5 columnas (si no, el
-- ON CONFLICT de 4 columnas ya no casa con ningún índice y rompe). Resto idéntico a 0130.
