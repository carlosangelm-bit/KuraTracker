...
--   1)  staff ASIGNADO al paciente de la herida SÍ puede INSERTAR.
...
--   4c) otro staff asignado NO puede hacer UPDATE de la ajena.         [0110]
--   4d) el AUTOR sí puede hacer UPDATE de la suya.                     [0110]
...
--
-- El escenario 3 (DENY) es el único que prueba que la policy NIEGA. Por eso NO
-- depende de la misma herida que 1/4: busca una pareja (staff, herida) cualquiera
-- donde el staff NO esté asignado al paciente de esa herida. Si el seed no da esa
-- pareja, cae a un fallback: prueba el DENY con `v_other` ANTES de asignarlo
-- (verificando que no venía ya asignado, o el test mentiría).
--
...
declare
  ...
  v_other       uuid;   -- staff.id de otro staff (lo asignamos para 4x)
  v_other_uid   uuid;
  v_other_preassigned boolean := false;
  v_out         uuid;   -- staff.id NO asignado (pareja independiente para el 3)
  v_out_uid     uuid;
  v_out_wound   uuid;   -- herida cuyo paciente v_out NO tiene asignado
  v_row         uuid;
  v_deleted     int;
  v_updated     int;
...
begin
  ...
  -- Otro staff (no admin/master) con perfil, distinto del autor: lo usaremos para 4x.
  ...
  -- ¿v_other ya venía asignado a ESTE paciente desde el seed? (importa para el
  -- fallback del escenario 3: si ya estaba, no sirve para probar el deny).
  if v_other is not null then
    v_other_preassigned := exists (
      select 1 from public.staff_patient_assignments a
      where a.staff_id = v_other and a.patient_id = v_patient);
  end if;

  -- ESCENARIO 3 (independiente): CUALQUIER pareja (staff, herida) con el staff
  -- NO asignado al paciente de esa herida. No tiene que ser v_wound.
  select s.id, s.profile_id, wpick.wid
    into v_out, v_out_uid, v_out_wound
  from public.staff s
  join public.profiles p on p.id = s.profile_id
  cross join lateral (
    select w.id as wid
    ...
    where not exists (
      select 1 from public.staff_patient_assignments a
      where a.staff_id = s.id and a.patient_id = w.patient_id)
    limit 1
  ) wpick
  ...
  r := r || format('Entidades: wound=%s autor=%s otro=%s (preasignado=%s) | deny: staff=%s wound=%s%s',
                   v_wound, v_author, v_other, v_other_preassigned, v_out, v_out_wound, E'\n');
  ...
  -- ---------------------------------------------------------------------------
  -- 3) INSERT denegado para un staff NO asignado.  *** ANTES de asignar v_other ***
  if v_out_uid is not null then
    -- Pareja independiente (staff, herida) no asignada. Camino preferido.
    perform set_config('request.jwt.claims',
    ...
    begin
      ...
      values (v_out_wound, 'granulacion', 'test-rls',
        '[{"x":1,"y":1,"engine_class":0,"lab":[50,0,0]}]'::jsonb,
        v_out_uid, 'clinico');
      r := r || E'FAIL 3 · staff NO asignado LOGRÓ insertar (RLS no bloqueó).\n';
    exception
      when insufficient_privilege then
        r := r || E'PASS 3 · RLS bloqueó el INSERT del staff NO asignado (pareja independiente).\n';
      when others then
        r := r || format('PASS? 3 · INSERT bloqueado con error inesperado: %s%s', sqlerrm, E'\n');
    end;
    reset role;
  elsif v_other is not null and not v_other_preassigned then
    -- Fallback: v_other AÚN no está asignado (verificado arriba). Probamos el deny
    -- sobre v_wound con v_other, y solo DESPUÉS lo asignamos para 4x.
    perform set_config('request.jwt.claims',
    ...
    begin
      ...
      values (v_wound, 'granulacion', 'test-rls',
        '[{"x":1,"y":1,"engine_class":0,"lab":[50,0,0]}]'::jsonb,
        v_other_uid, 'clinico');
      r := r || E'FAIL 3 · staff NO asignado (fallback v_other) LOGRÓ insertar.\n';
    exception
      when insufficient_privilege then
        r := r || E'PASS 3 · RLS bloqueó el INSERT (fallback: v_other antes de asignarlo).\n';
      when others then
        r := r || format('PASS? 3 · INSERT bloqueado con error inesperado: %s%s', sqlerrm, E'\n');
    end;
    reset role;
  else
    r := r || E'SKIP 3 · seed degenerado: no hay ninguna pareja (staff, herida) no asignada. Añade un paciente/staff sin asignación cruzada al seed.\n';
  end if;

  -- Ahora sí: asegurar que "otro" queda asignado (para 4a-4d). Rollback lo deshace.
  if v_other is not null then
  ...
  -- 4c) ... ni hacerle UPDATE.  (La fila v_row sobrevive a 4a y 4c.)
  if v_row is null then
    r := r || E'SKIP 4 · no se creó la fila base (falló el paso 1).\n';
  elsif v_other_uid is null then
    r := r || E'SKIP 4a/4c · no hay un segundo staff con perfil para probar autor-vs-no-autor.\n';
  else
    perform set_config('request.jwt.claims',
      json_build_object('sub', v_other_uid::text, 'role', 'authenticated')::text, true);
    set local role authenticated;
    ...
    update public.wound_vision_corrections set note = 'ajena' where id = v_row;
    get diagnostics v_updated = row_count;
    reset role;
    r := r || case when v_deleted = 0
      then E'PASS 4a · otro staff asignado NO borró la corrección ajena.\n'
      else format('FAIL 4a · otro staff BORRÓ corrección ajena (%s fila).%s', v_deleted, E'\n') end;
    r := r || case when v_updated = 0
      then E'PASS 4c · otro staff asignado NO pudo hacer UPDATE de la ajena.\n'
      else format('FAIL 4c · otro staff ACTUALIZÓ corrección ajena (%s fila).%s', v_updated, E'\n') end;
  end if;

  -- 4d) El AUTOR sí puede hacer UPDATE de la suya; 4b) y borrarla.
  if v_row is not null then
    perform set_config('request.jwt.claims',
      json_build_object('sub', v_author_uid::text, 'role', 'authenticated')::text, true);
    set local role authenticated;
    update public.wound_vision_corrections set note = 'propia' where id = v_row;
    get diagnostics v_updated = row_count;
    delete from public.wound_vision_corrections where id = v_row;
    get diagnostics v_deleted = row_count;
    reset role;
    r := r || case when v_updated = 1
      then E'PASS 4d · el autor SÍ pudo hacer UPDATE de la suya.\n'
      else format('FAIL 4d · el autor no pudo actualizar la suya (%s filas).%s', v_updated, E'\n') end;
    r := r || case when v_deleted = 1
      then E'PASS 4b · el autor SÍ borró la suya.\n'
      else format('FAIL 4b · el autor no pudo borrar la suya (%s filas).%s', v_deleted, E'\n') end;
  end if;
