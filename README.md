...
| Fix "pantalla en blanco" al crear concepto de catálogo | ✅ Corregido — `NoteOptionCatalogItem.fromJsonOrNull()` descarta filas de `note_option_catalog` malformadas (id/label nulos) en vez de lanzar sin capturar durante el rebuild; además, `ErrorWidget.builder` global muestra el error+stack en pantalla ante cualquier excepción de build no prevista (ver sección 3.4c) |
...
*Última actualización: Fix del bug "pantalla en blanco al crear concepto de catálogo" (reportado contra Supabase real) + hardening global de errores de UI. Diagnóstico basado en evidencia real (stack traces reales capturados en `test/unit/note_catalog_blank_screen_repro_test.dart`, sin acceso a producción): la hipótesis de hueco de RLS (`insertRow().select().single()`) se descarta como causa única porque, de ocurrir, cae dentro del `try/catch` de `_addOption()` (produciría SnackBar); el mecanismo que sí reproduce el síntoma exacto (sin SnackBar) es que `_NoteCatalogTabState.build()` llama `listAllNoteOptions()` sin protección tras el `setState()`, y esa función mapeaba TODA la colección cacheada con `NoteOptionCatalogItem.fromJson()` (casts no-nulos de id/label) — una sola fila cacheada malformada (no necesariamente la recién creada) lanza un `TypeError` fuera del alcance del try/catch. Fix: `NoteOptionCatalogItem.fromJsonOrNull()` (valida por tipo real, devuelve null en vez de lanzar) + `listNoteOptions()`/`listAllNoteOptions()` actualizados para descartar filas corruptas en silencio. Hardening independiente y explícitamente solicitado: `ErrorWidget.builder` global en `lib/main.dart` muestra excepción+stack en pantalla ante cualquier fallo de build no previsto, en vez de pantalla en blanco. Nota de alcance: no se pudo reproducir el bug contra el Supabase real de producción desde este sandbox (sin credenciales disponibles); si el cliente reproduce el bug de nuevo tras este fix, el hardening ahora mostrará el error real en pantalla, lo que permitirá diagnosticar cualquier causa adicional (p.ej. relacionada con RLS/`organization_id`, migración `0011` aún pendiente de aplicar — ver sección 9 paso 0) sin depender de la consola del navegador. Build/analyze (0 errores)/tests (58/58, incluye 6 nuevos) verificados, demo redesplegada en sandbox de desarrollo.*
