...
*Última actualización: La gráfica "Composición del lecho en el tiempo (%)" del tablero de seguimiento pasó de 4 líneas independientes 0-100% (granulación/esfacelo/necrosis/epitelización, potencialmente superpuestas sin relación entre sí) a un verdadero **gráfico de líneas 100% apiladas**: en cada visita las 4 franjas se apilan de abajo hacia arriba y siempre cubren el 100% de la altura, mostrando la proporción real de cada clasificación de tejido. Técnica: `fl_chart` no tiene un tipo "stacked" nativo para `LineChart`, así que se calculan 4 líneas de suma acumulada (`bedCompositionCumulative()`, normalizando defensivamente a 100 por si la captura no suma exactamente 100) y se rellenan las 4 franjas entre líneas consecutivas con `betweenBarsData`, reutilizando los mismos `KuraColors` del legend existente. El tooltip muestra los valores reales (no acumulados) de cada tejido en la visita tocada. 5 tests nuevos (`test/unit/bed_composition_stacked_chart_test.dart`), suite completa 158/158, `flutter analyze` limpio de errores, `build_gate.sh` verde. No se tocó el modelo de datos ni el motor de reglas. Mergeado a `main` (commit `4889d32`) con visto bueno del usuario; producción y demo reconstruidas y verificadas (curl 200 en `/` y `/patients` en ambos puertos 3000/3001).*

*Actualización previa: Agregado cálculo de volumen editable por fórmula de Kundin (`volume_cm3 = Largo × Ancho × Profundidad × 0.327`) en ambas pantallas de captura (valoración y seguimiento), con detección/anotación de ajuste manual ("✎ Volumen ajustado manualmente") derivada por tolerancia (no un toggle persistido) en captura, detalle de consulta y tooltip de gráfica; migración additiva `0015_wound_measurements_volume_manual.sql` (columna `volume_manual`, pendiente de aplicación manual por Carlos); dos gráficas nuevas y additivas en el tablero de seguimiento (volumen en el tiempo con placeholder para heridas sin datos 3D, y composición del tejido en el tiempo) sin modificar la gráfica de área existente. 15 tests nuevos (`test/unit/wound_volume_test.dart`), suite completa 150/150, `flutter analyze` limpio de errores, `build_gate.sh` verde. No cambia lógica del motor de reglas (`rulesVersion` sin bump). Todo el trabajo vive en la rama `feat/volume-kundin-charts`, no en `main`, pendiente del visto bueno explícito del usuario para el merge — ver sección 3.4f.*
