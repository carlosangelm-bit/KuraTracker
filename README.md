...
*Última actualización: Agregado el rol `master` (administrador de plataforma) — migración `0012_master_role.sql` (versionada, pendiente de aplicación manual), `PlatformHomeScreen` con selector de centro reutilizando `StaffTab`/`SitesTab`/`NoteCatalogTab`, ruta `/platform` y nav exclusiva. 67/67 tests pasando (6 nuevos), `flutter build web --release` exitoso, verificado en vivo con Playwright (incluye un bug real de `RangeError` en avatar de personal sin folio, encontrado y corregido durante esta verificación). Todo el trabajo vive en la rama `feat/master-role`, no en `main`, hasta confirmación del usuario — ver sección 5.4. Pendiente: prueba negativa de privacidad contra Supabase real (requiere que se apliquen las migraciones 0011/0012 primero) y el bootstrap manual del primer master.*
